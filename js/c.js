...
(function() {
  ...
  doTOTPRemove = function() {
    var ajaxLanding, args, remove_totp, urlString;
    noSubmit();
    animateLoad();
    url = $.url();
    ajaxLanding = "async_login_handler.php";
  ...
  makeTOTP = function() {
    ...
    totp.done(function(result) {
      ...
      if (result.status === true) {
        ...
        html = "<form id='totp_verify'> <p>To continue, scan this barcode with your smartphone authenticator application. <small><a href='#' id='alt_totp_help'>Don't have the app?</a></small></p> <p style='font-weight:bold'>If you're unable to do so, <a href='#' id='" + show_secret_id + "'>click here to manually input your key.</a></p> <div id='" + barcodeDiv + "'> " + result.svg + " <p>Don't see the barcode? <a href='#' id='" + show_alt + "'>Click here</a></p> </div> <p>Once you've done so, enter the code generated by your app in the field below to verify your setup.</p> <fieldset> <legend>Confirmation</legend> <input type='number' size='6' maxlength='6' id='code' name='code' placeholder='Code'/> <input type='hidden' id='username' name='username' value='" + user + "'/> <button id='verify_totp_button' class='totpbutton'>Verify</button> </fieldset> </form>";
        $("#totp_add").html(html);
